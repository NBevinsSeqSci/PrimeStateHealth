...
export default function TrailsTest({
  ...
  const finalizeAndSubmit = (finalTimeMs: number | null) => {
    const totalTimeSec =
      finalTimeMs !== null && startTimeMs !== null
        ? (finalTimeMs - startTimeMs) / 1000
        : null;
    const adjustedTimeSec =
      totalTimeSec !== null ? totalTimeSec + errors * errorPenaltySec : null;

    const result: TrailsResult = {
      totalTimeSec:
        totalTimeSec !== null ? Math.round(totalTimeSec * 10) / 10 : null,
      adjustedTimeSec:
        adjustedTimeSec !== null ? Math.round(adjustedTimeSec * 10) / 10 : null,
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-4 text-center">
      <div className="space-y-2">
        <h3 className="text-2xl font-semibold text-slate-900">Set-Shifting Trails</h3>
        ...
        <p className="text-xs text-slate-500">
          ...
          <span className="font-semibold">
            {SEQUENCE[nextExpectedIndex]?.label ?? "Done"}
          </span>
        </p>
      </div>

      <div
        className={`relative aspect-square rounded-xl border border-slate-200 shadow-inner transition-colors ${
          flash === "correct"
            ? "bg-emerald-50"
            : flash === "incorrect"
              ? "bg-red-50"
          ...
          return (
            <button
              ...
              className={`absolute w-10 h-10 -ml-5 -mt-5 rounded-full border-2 flex items-center justify-center font-bold text-lg transition-all ${
                ...
                isNext
                  ? "ring-4 ring-slate-300 scale-110 border-slate-900"
                  : ""
              }`}
      ...
      <p className="text-xs text-slate-500">
        Points: {SEQUENCE.length} &bull; Errors: {errors}
      </p>
    </div>
  );
}
